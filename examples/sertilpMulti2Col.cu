...
// Slice Ellpack-R format for storing sparse matrix, uses ILP - prefetch vector elements in registers
...
extern "C" __global__ void rbfSERTILP2multi_class(const float * vals,
							   const int * colIdx, 
							   const int * rowLength, 
							   const int * sliceStart,
							   const float* selfDot,
							   ...
							   const int idxJ_ds,
							   const int cls1_N_aligned, //aligned to power of 2 class 1 size
							   const int * cls_start, //arry of integer pointers where each class starts
							   const int * cls_count, //2-element array, number of elements in each class
							   const int * cls, // 2-element array, with class number eg. cls[0]=2, cls[1]=5, binary classifiers between class 2 vs 5
							   const int * cls_slice_start //array of integer indicators from which slice particular class begins eg. cls_slice_start[2]==12 means that class 2 starts from slice 12, slice == block
							   )
{
	...
	__shared__ int shYJ;
	__shared__ int shClsSum;
	__shared__ int th_cls;
	__shared__ int class_block;
	...
	if(threadIdx.x==0)
	{
		...
		//determines the class membership, (first cls1_N threads belongs to first class),0 - first class, 1- second class
		th_cls = (blockIdx.x+1)*SLICE_SIZE <= cls1_N_aligned ? 0:1; 
		//it should also work, without adding one
		//th_cls = blockIdx.x*SLICE_SIZE < cls1_N ? 0:1; 
		
		//slice number from which each class starts
		class_block = cls_slice_start[cls[th_cls]]+blockIdx.x-th_cls*(cls1_N_aligned/SLICE_SIZE);
	...
	{		
		...
		for(unsigned int i=0; i<maxEl;i++)
		{
			...
			for( j=0; j<PREFETCH_SIZE;j++)			
			{
				...
				arIdx = (i*PREFETCH_SIZE+j )*align+sliceStart[class_block]+threadIdx.x;
		...
		if(threadIdx.x%THREAD_PER_ROW==0)
		{
			...
			//it is ok?, row is in global dataset, and rIdx is mapped into 2 classes
